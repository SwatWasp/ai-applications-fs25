...
# Load dataset
df_bfs_data = pd.read_csv('bfs_all_tax_data.csv', sep=',', encoding='utf-8')
...
def predict_apartment(rooms, area, town):
    ...
    if len(df) != 1: # if there are more than two records with the same bfs_number reutrn -1
        return -1
    prediction = random_forest_model.predict(df[['rooms', 'area', 'pop', 'pop_dens', 'frg_pct', 'emp', 'tax_income', 'mean_tax']])
